...
#         sql = '''INSERT INTO translate_reports 
#                 (name, mailing_name, phone, email, law_society_number, business_name, trusteeships, current_practice_restrictions)
...
#             data['phone'],
#             data['email'],
#             data['Law Society Number'],
#             data['Business Name'],
#             data['Trusteeships'],
#             data['Current Practice Restrictions']
#         ))
#         conn.commit()
...
#         }
#         # cur.execute("INSERT into translate_reports(name,mailing_name,phone,email,law_society_number,business_name,trusteeships,current_practice_restrictions)")
...
#     #     print("Data appended to a new file.")
#     # else:
#     #     try:
...
#     #             print("Data appended to an existing file.")
#     #         else:
#     #             print("Existing data is empty or None. Appending new data directly.")
#     #             new_data = pd.DataFrame(data_list)
#     #             new_data.to_excel(file_path, index=False)
#     #             print("New data saved to the file.")
#     #     except Exception as e:
#     #         print("Error occurred while reading existing data:", e)
...
def extract_lawyer_info(soup, link):
    ...
    # Extract status
    status = soup.find('div', class_='label-wrapper member-info-label')
    status_text = status.find_next('div', class_='member-info-value').text.strip() if status else "Status information not found"
...
# Define file path
FILE_PATH = os.path.dirname(os.getcwd()) + r"\Canada_link2"
...
while i < count:
    ...
    for link in links:
        ...
        lawyer_info = extract_lawyer_info(soup, link)
        if lawyer_info is not None:
            cur.execute("INSERT INTO translate_reports (name, mailing_name, phone, email, law_society_number, business_name, trusteeships, current_practice_restrictions, current_regulatory_proceedings, regulatory_history, offers_services_in_french, business_address) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                        (lawyer_info['Name'], lawyer_info['Mailing Name'], lawyer_info['phone'], lawyer_info['email'], lawyer_info['Law Society Number'], lawyer_info['Business Name'], lawyer_info['Trusteeships'], lawyer_info['Current Practice Restrictions'], lawyer_info['Current Regulatory Proceedings'], lawyer_info['Regulatory History'], lawyer_info['Offers Services in French?'], lawyer_info['Business Address']))
