...
def get_listed_object(record):
    ...
    meta_detail['renewal_date'] = timestamp_to_str(record['renewal_date']) if record['renewal_date'] else '' 


    address_details = [
        {
            'type': 'general_address',
            'address': record['address'].replace("NONE,", '').replace("none,", '').replace("None,", ''),
    ...
    data_obj = {
        ...
        "dissolution_date": "",
        "type": record['type'],
        "crawler_name": "custom_crawlers.kyb.indiana.indiana_kyb_status",
    ...
    if len(incorporator_table_rows) >= 1:
        for people in incorporator_table_rows[1:]:
            ...
            if len(cells) > 1:
                ...
                people['name'] = cells[1].text.strip().replace("'", "''").replace("NONE", "").replace("none", "").replace("None", "") if cells[2] else ''
                people['address'] = cells[2].text.strip().replace("'", "''").replace("NONE", "").replace("..", "").replace("none", "").replace("None", "") if cells[2] else ''
    ...
    if len(governing_table_rows) >= 1:
        for people in governing_table_rows[1:]:
            ...
            if len(cells) > 1:
                people = dict()
                people['designation'] = cells[0].text.strip().replace("'", "''")
                people['name'] = cells[1].text.strip().replace("'", "''").replace("NONE", "").replace("..", "").replace("none", "").replace("None", "") if cells[2] else ''
                people['address'] = cells[2].text.strip().replace("'", "''").replace("NONE", "").replace("..", "").replace("none", "").replace("None", "") if cells[2] else ''
            people_list.append(people)    
...
def get_data_from_table(driver, total_page, source_type, entity_type, country, category, url, name, description):
    ...
    # Read existing anchor tags from the file and store them in a set
    with open('status_anchors.txt', 'r') as file:
        for line in file:
            existing_anchors.add(line.strip())

    with open('status_pagination.txt', 'r') as file:
        for line in file:
            existing_pagination.add(line.strip())

    with open('status_business_ids.txt', 'r') as file:
        for line in file:
            existing_business_ids.add(line.strip())

    for i in range(page_number, total_page+1):
        ...
        if str(i) not in existing_pagination:
            ...
            with open('status_pagination.txt', 'a') as file:
                file.write(str(i) + '\n')
    ...
    with open('status_anchors.txt', 'r') as file:
        for line in file:
            ...
            anchor_tag = tag_soup.find('a')

            if cfbm_cookies:
                cookies = f"ASP.NET_SessionId={session_id};__cf_bm={cfbm_cookies};__RequestVerificationToken={request_verification_token};"
            businessid = urllib.parse.quote(anchor_tag.get('businessid'))
            businesstype = urllib.parse.quote(anchor_tag.get('businesstype'))
            id = urllib.parse.quote(anchor_tag.get('id'))
            isseries = urllib.parse.quote(anchor_tag.get('isseries'))
            ...
            if str(businessid) not in existing_business_ids:
                ...
                data['inactive_date'] = inactive_date_td.find('strong').text.strip()  if inactive_date_td and inactive_date_td.find('strong') else ''

                principal_office_address_el = soup.find("td", string="Principal Office Address:") if soup.find("td", string="Principal Office Address:") else soup.find("td", string="Applicant Address:")
                ...
                renewal_date_el = soup.find("td", string=re.compile("Renewal Date:")) if soup.find("td", string=re.compile("Renewal Date:")) else ''
                renewal_date_td = renewal_date_el.find_next('td') if renewal_date_el else ''
                data['renewal_date'] = renewal_date_td.find('strong').text.strip() if renewal_date_td and renewal_date_td.find('strong') else ''
                ...
                data['formation_date'] = original_formation_date_td.find('strong').text.strip() if original_formation_date_td and original_formation_date_td.find('strong') else ''
                ...
                if registered_agent:
                    ...
                    registered_agent_obj['meta_detail'] = {
                        'entity_type': registered_agent_type_el.find_next('td').text.strip().replace("'", "''") if registered_agent_type_el else ''
                    }
                ...
                with open('status_business_ids.txt', 'a') as file:
...
def get_records(source_type, entity_type, country, category, url, name, description):
    ...
    try:
        ...
        for v in selected_values:
            ...
            while True:
                try:
                    if len(driver.find_elements(By.XPATH, '//iframe[@title="reCAPTCHA"]')) == 0:
                        continue
            ...
            type_dropdown = Select(driver.find_element(By.ID, 'BusinessStatus'))
            ...
            get_data_from_table(driver,tota_page, source_type, entity_type, country, category, url, name, description)

        file_paths = ["status_pagination.txt", "status_anchors.txt", "status_business_ids.txt"]
