...
crawler_config = {
    ...
    'CRAWLER_NAME': "Arizona_1 Official Registry"
}
...
arguments = sys.argv
ENTITY_NUM = int(arguments[1]) if len(arguments)>1 else start_num

all_range_num = [f"{str(i).zfill(7)}" for i in range(ENTITY_NUM, end_num + 1)]
        ...
        for button in buttons[:1]:
            ...
            try:
                ...
                for row_ in table2.find_all('tr', class_='bgwhite'):
                    ...
                    fillings_dict = {
                        "filing_type":document_type.replace("%","%%"),
