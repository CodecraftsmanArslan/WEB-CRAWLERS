...
from proxies_list import get_a_random_proxy
...
arguments = sys.argv
PAGE_NUMBER = arguments[1] if len(arguments) > 1 else 60000
...
def make_request(url, headers, timeout, proxy):
    while True:
...
def make_post_request(url, headers, payload, timeout, proxy):
    while True:
        max_retries = 3
        for retry in range(max_retries):
            try:
                response = requests.post(
                    ...
                    proxies={
                        "http": os.getenv("THAILAND_PROXY_1"),
                        "https": os.getenv("THAILAND_PROXY_1")
                    })
                return response
            except ProxyError:
                PROXY_ERROR = True
...
def get_records(source_type, entity_type, country, category, url, name, description):
    ...
    try:
        ...
        proxy = get_a_random_proxy()
        while True:
            response = make_request(BASE_URL, base_headers, 200,proxy)
            if not response:
                proxy = get_a_random_proxy()
            else:
                break
        ...
        while page_number <= int(total_pages):
            ...
            response = make_post_request(
                API_URL, headers, payload['body'].format(page_number, total_pages), 120, proxy)
            if not response:
                continue
            STATUS_CODE = response.status_code
            DATA_SIZE = len(response.content)
                ...
                for row in rows:
                    ...
                    while True:
                        response = make_request(
                            PREFIX_URL.format(company_link), headers, 200,proxy)
                        if not response:
                            proxy = get_a_random_proxy()
                        else:
                            break
                    ...
                    match = re.search(r'\\profile\\tab1\\(.*?)";', response.text.replace("\/", "\\"))
                    if match:
                    ...
                    while True:
                        profile_response = make_request(
                            PROFILE_URL.format(company_id), headers, 120, proxy)
                    ...
                    while True:
                        financial_summary_response = make_request(Financial_Summary_URL.format(company_id), headers, 120,proxy)
                        if not financial_summary_response:
                            proxy = get_a_random_proxy()
                        else:
                            break
                ...
                #   Extracting Submission History
                    while True:
                        submission_history_response = make_request(
                            Submission_History_URL.format(company_id), headers, 12, proxy)
                        if not submission_history_response:
                            proxy = get_a_random_proxy()
                        else:
                            break                    
                ...
                #   Extracting Investment by Nationality
                    while True:
                        information_response = make_request(
                            INFORMATION_URL.format(company_id), headers, 120, proxy)
                        if not information_response:
                            proxy = get_a_random_proxy()
                        else:
                            break                    
                ...
                #   Extracting Previous Name Details
                    while True:
                        change_response = make_request(
                            CHANGE_URL.format(company_id), headers, 120, proxy)
