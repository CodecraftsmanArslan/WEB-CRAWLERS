...
def get_listed_object(record):
    ...
    additional_data = [
        {
        "type": "beneficiary_owner",
        "data":
            [
                {
                  "name":  str(record["Owners"]).replace("'","''")
                }
            ]
        }
    ]
    ...
    physical_address['type'] = "general_address"
    physical_address['address'] = str(record["PhysicalLine1"]).replace("'","''").replace("none","").replace("NONE","").replace("None","")+' '+str(record["PhysicalLine2"]).replace("'","''").replace("none","").replace("NONE","").replace("None","")+' '+record["PhysicalCountry"].replace("'","''").replace("none","")+' '+record["PhysicalCity"].replace("'","''").replace("none","").replace("None","")+' '+record["PhysicalState"].replace("'","''").replace("none","")+' '+ record["PhysicalZipOut"]
    mailing_address = dict()
    mailing_address['type'] = "postal_address"
    mailing_address['address'] = str(record["MailingLine1"]).replace("'","''").replace("none","").replace("NONE","").replace("None","")+' '+str(record["MailingLine2"]).replace("'","''").replace("none","").replace("NONE","").replace("None","")+' '+record["MailingCountry"].replace("'","''").replace("none","")+' '+record["MailingCity"].replace("'","''").replace("none","")+' '+record["MailingState"].replace("'","''").replace("none","")+' '+record["MailingZipOut"].replace("none","")
    ...
    data_obj = {
        ...
        "registration_number": str(record["LicenseNumber"]),
        "registration_date": str(pd.to_datetime(record["IssueDate"])).replace("NaT","") ,
        "dissolution_date": "",
        "type": "",
        "crawler_name": "custom_crawlers.kyb.alaska.alaska_business_lincese_kyb",
        "country_name": "Alaska",
...
def prepare_data(record, category, country, entity_type, type_, name_, url_, description_):
    ...
    '''
    data_for_db = list()
    data_for_db.append(shortuuid.uuid(str(record["LicenseNumber"])+record["BusinessName"])) # entity_id
...
def get_records(source_type, entity_type, country, category, url, name, description):
    ...
    try:
        ...
        df = pd.read_csv('.//kyb/alaska/input/BusinessLicenseDownload.CSV')
...
if __name__ == '__main__':
    ...
    url = "https://www.commerce.alaska.gov/cbp/DBDownloads/BusinessLicenseDownload.CSV"
