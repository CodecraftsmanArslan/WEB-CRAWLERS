...
def get_listed_object(record):
    ...
    meta_detail = dict()
    meta_detail["classification"] = str(record["CLASSIFICATION"]).replace("'","''") 
    meta_detail["expiration_date"] = str(record["DURATIONEXPIRATIONDATE"]).replace("'","''").replace("/","-") 
    meta_detail["formation_date"] = str(record["AKFORMEDDATE"]).replace("'","''").replace("/","-")  
    
    people_detail = [
        {
        'designation': "owner",
        "name": record["OwnerFirstname"].replace("'","''")+''+record["OwnerLastname"].replace("'","''")+''+record["OwnerEntityname"].replace("'","''"),
        "address":str(record['OwnerAddress']).replace("'","''") +' '+str(record['OwnerAddress2']).replace("'","''")+' '+record["OwnerCountry"]+' '+record["OwnerCity"] +' '+record["OwnerState"]+' '+record["OwnerZip"]
    }
    ]
   
    addresses_detail = []
    mailing_address = dict()
    mailing_address['type'] = 'general_address'
    mailing_address['address'] =str(record['ENTITYMAILINGADDRESS1']).replace("'","''").replace("none","") +' '+str(record['ENTITYMAILINGADDRESS2']).replace("'","''").replace("none","")+' '+record["ENTITYMAILINGCITY"]+' '+record["ENTITYMAILINGCOUNTRY"]+' '+record["ENTITYMAILINGSTATEPROVINCE"]+' '+record["ENTITYMAILINGZIP"]
    mailing_address['description'] = ""
    mailing_address['meta_detail'] = {}
    addresses_detail.append(mailing_address)
    
    # create data object dictionary containing all above dictionaries
    data_obj = {
        
        "name": record["MarkName"].replace("'","''"),
        "status": "",
        "registration_number": str(record["MarkNumber"]),
        "registration_date": str(pd.to_datetime(record['AKFORMEDDATE'])),
        "dissolution_date": "",
        "type": record["MarkType"],
        "crawler_name": "custom_crawlers.kyb.alaska.alaska_trademark_kyb",
        "country_name": "Alaska",
...
def prepare_data(record, category, country, entity_type, type_, name_, url_, description_):
    ...
    '''
    data_for_db = list()
    data_for_db.append(shortuuid.uuid(record["MarkType"]+record["MarkNumber"]+record["MarkName"])) # entity_id
    data_for_db.append(record["MarkName"].replace("'", "")) #name
...
def get_records(source_type, entity_type, country, category, url, name, description):
    ...
    try:
        ...
        df = pd.read_csv('.//kyb/alaska/input/TrademarkDownload.csv')
...
if __name__ == '__main__':
    ...
    url = "https://www.commerce.alaska.gov/cbp/DBDownloads/TrademarkDownload.csv"
