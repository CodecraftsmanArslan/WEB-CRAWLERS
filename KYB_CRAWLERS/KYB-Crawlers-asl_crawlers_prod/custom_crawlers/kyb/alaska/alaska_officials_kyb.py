...
def get_listed_object(record):
    ...
    meta_detail = dict()
    people_detail = dict()
    people_detail['name'] = record["OfficialFirstName"].replace("'","''")+''+record["OfficialLastOrEntityName"].replace("'","''")
    people_detail['designation'] = record["OfficialTitle"].replace("'","''")
    people_detail['meta_detail'] = {"person_registration_number" : str(record['OfficialEntityNumber']).replace("'","''") }
        
    # create data object dictionary containing all above dictionaries
    data_obj = {
        
        "name": record["ParentEntityName"].replace("'","''"),
        "status": "",
        "registration_number": str(record['ParentEntityNumber']),
        "registration_date": "",
        "dissolution_date": "",
        "type": record["ParentRecordType"],
        "crawler_name": "custom_crawlers.kyb.alaska.alaska_officials_kyb",
        "country_name": "Alaska",
...
def prepare_data(record, category, country, entity_type, type_, name_, url_, description_):
    ...
    '''
    data_for_db = list()
    data_for_db.append(shortuuid.uuid(record["ParentEntityName"]+record["OfficialEntityNumber"]+record["ParentEntityNumber"])) # entity_id
    data_for_db.append(record["ParentEntityName"].replace("'", "")) #name
...
def get_records(source_type, entity_type, country, category, url, name, description):
    ...
    try:
        ...
        df = pd.read_csv('.//kyb/alaska/input/OfficialsDownload.csv')
...
if __name__ == '__main__':
    ...
    url = "https://www.commerce.alaska.gov/cbp/DBDownloads/OfficialsDownload.csv"
