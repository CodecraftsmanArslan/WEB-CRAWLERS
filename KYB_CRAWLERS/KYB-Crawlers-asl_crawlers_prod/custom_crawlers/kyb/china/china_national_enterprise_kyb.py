...
def get_listed_object(record):
    ...
    meta_detail = dict()
    meta_detail['industry_type'] = "其他行业"
    meta_detail['address'] = record[2].replace("'","''")
    meta_detail['number_of_employees'] = "50人 不含 以下"
    meta_detail['capital'] = record[3].replace("'","''")
    meta_detail['court_of_acceptance'] = record[4].replace("'","''")
    meta_detail['business_scope'] = record[5].replace("'","''")
    meta_detail['company_profile'] = record[-1].replace("'","''")
    meta_detail['debtor_details'] = ""
    meta_detail['related_announcements'] = record[-2].replace("'","''")
    ...
    data_obj = {
        ...
        "dissolution_date": "",
        "type": record[1],
        "crawler_name": "crawlers.custom_crawlers.insolvency.china_national_enterprise_kyb",
        "country_name": "China",
...
def prepare_data(record, category, country, entity_type, type_, name_, url_, description_):
    ...
    '''
    data_for_db = list()
    data_for_db.append(shortuuid.uuid(record[0]+record[1]+record[2]+record[3])) # entity_id
...
def get_records(source_type, entity_type, country, category, url, name, description):
    ...
    try:
        ...
        DATA = []
        contant_name = soup.find('h2').get_text().strip()
        DATA.append(contant_name)
        table = soup.find_all('table')[0]
        trs = table.find_all('tr')
        for tr in trs:
            tds = tr.find('td').text.split("：")[-1].split(":")[-1].strip()
            DATA.append(tds)
        company_profile = soup.find('div',class_ = "detail_text_con").text.strip()
        DATA.append(company_profile)
...
if __name__ == '__main__':
    ...
    df = pd.read_csv(".//kyb/china/inputs/china_pccz.csv")
