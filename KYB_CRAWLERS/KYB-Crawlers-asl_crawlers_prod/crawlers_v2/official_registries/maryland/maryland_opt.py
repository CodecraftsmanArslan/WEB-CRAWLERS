...
CONTENT_TYPE = 'HTML'
ARGUMENT = sys.argv
FILE_PATH = os.path.dirname(os.getcwd()) + "/maryland"

maryland_crawler = CustomCrawler(
...
# If there is a captcha, it will solve it.
...
alphabets = ["W", "F", "T", "M", "Z", "D", "L", "."]

# create a search query and search the record to scrape and store in database.


def crawl(alp_list):
    for letter in alp_list[alphabets.index(start_alph):alphabets.index(ending_alph)]:
        for i in range(0, 3):
            for numb in range(0, 10000000):
                ...
                name_number = soup.find(
                    'div', class_='legendStyle').get_text(strip=True)
                ...
                filing = driver.find_element(
                    By.XPATH, '//a[text()="Filing History"]')
                filing.click()
                time.sleep(2)
                filliings_detail = []
                ...
                for filing_row in filling_rows[2:]:
                    ...
                    try:
                        file_url = filing_cells[0].find_element(
                ...
                try:
                    if data_dict.get('Resident Agent:', ''):
                        matches = re.match(
                            pattern, data_dict.get('Resident Agent:', ''))
                        if matches:
                    ...
                    if data_dict.get('Owner:', ''):
                        ...
                        matches_ = re.match(
                            new_pattern_, data_dict.get('Owner:', ''))
                        if matches_:
                            designation_ = 'owner'
                ...
                # Get Annual Reports
                try:
                    annual_report = driver.find_element(
                        By.XPATH, '//a[text()="Annual Report/Personal Property"]')
                    annual_report.click()
                    time.sleep(5)
                    try:
                        mailing_address = driver.find_element(
                            By.XPATH, '//div/strong[text()="Mailing Address:"]/parent::div/following-sibling::div').text.strip()
                        mailing_addresses = mailing_address.split(
                            "\n")[2]+' '+mailing_address.split("\n")[-1]
                    except:
                        mailing_addresses = ""
                    annual_table = driver.find_element(
                        By.ID, 'tblPersProperty1')
                    ...
                    for annual_row in annual_rows[1:]:
                        annual_cells = annual_row.find_elements(
                            By.TAG_NAME, 'td')
                    ...
                    personal_table = driver.find_element(
                        By.ID, 'tblPersProperty2')
                    personal_rows = personal_table.find_elements(
                        By.TAG_NAME, 'tr')
                    for personal_row in personal_rows[1:]:
                        personal_cells = personal_row.find_elements(
                            By.TAG_NAME, 'td')
                ...
                location_match = re.match(
                    new_pattern, data_dict.get('Location:', ''))
                location = ""
                try:
                    if location_match:
                ...
                ENTITY_ID = maryland_crawler.generate_entity_id(
                    OBJ['registration_number'], OBJ['name'])
                BIRTH_INCORPORATION_DATE = ''
                ROW = maryland_crawler.prepare_row_for_db(
...
try:
    start_alph = ARGUMENT[1].upper() if len(ARGUMENT) > 1 else alphabets[0]
    ending_alph = ARGUMENT[2].upper() if len(ARGUMENT) > 2 else alphabets[-1]

    crawl(alp_list=alphabets)
