...
options.add_argument(f'--proxy-server=http://103.37.182.154:6205')
...
def extract_company_items(driver):
    try:
...
def extract_people_details(driver):
    people_details = []
    ...
    try:
        ...
        board_name = peoples_soup1.find('span').text.strip()
        board_address = peoples_soup1.find_all('div')[1].text.strip()   
...
def extract_filling_detail(driver):
    filling_detail = []
    try:
        ...
        print(f"Error in get_previous_names_details: {e}")
        filling_detail = []


    return filling_detail


def extract_additiona_detail(driver):
    additiona_detail =  []
    try:
    ...
    return additiona_detail


def extract_previous_names_detail (driver):
...
def get_number_of_employees_details(driver):
    additiona_detail=[]
    try:
...
def scrape_data(driver, start_number, end_number):
    for search_number in range(start_number, end_number):
        url = f"https://datacvr.virk.dk/enhed/virksomhed/{search_number}?fritekst={search_number}&sideIndex=0&size=10"
        driver.get(url)

                    # Check if the company exists
        if "Virksomheden kunne ikke findes" in driver.page_source:
        ...
        db_items = extract_db_items(driver)
        company_items = extract_company_items(driver)
        people_details = extract_people_details(driver)
        filling_detail = extract_filling_detail(driver)
        additiona_detail = extract_additiona_detail(driver)
        previous_names_detail = extract_previous_names_detail(driver)
        additiona_detail = get_number_of_employees_details(driver)
        ...
        OBJ = {
            ...
            "accounting_period":" ",
            "drawing_rule":people_details.get('Tegningsregel',''),
...
if __name__ == '__main__':
    scrape_data(driver, start_number, end_number)
